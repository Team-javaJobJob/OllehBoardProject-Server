...
@CrossOrigin(origins = "http://localhost:3000") // client 의 url
...
//    }
    ...
    @GetMapping("/list")
    public ResponseEntity<Page<Post>> findAllPost(@PageableDefault(size = 10, sort = "member_id", direction = Sort.Direction.ASC) final Pageable pageable) {
        log.info("GET /api/v1/boards");
        Page<Post> postDTOList = postService.findAllBoards(pageable);
        return new ResponseEntity<>(postDTOList, HttpStatus.OK);
    }
