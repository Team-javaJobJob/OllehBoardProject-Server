...
public class CommunityService {
    ...
    @Transactional
    public CommunityDTO updateCommunity(Long communityId, CommunityUpdateRequest communityUpdateRequest, MultipartFile file, Authentication authentication) throws Exception {
        ...
        // 수정 전 기존 키워드 삭제
        keywordService.delete(CommunityDTO.fromEntity(community));
        // 수정할 키워드 저장
        keywordService.saveKeywordToUpdateCommunity(communityUpdateRequest, community);

        // 수정 전 기존 이미지 삭제
        imageService.delete(CommunityDTO.fromEntity(community));
        // 수정할 이미지 저장
    ...
    private void deleteByCommunity(Community community) {
        communityMemberService.delete(CommunityDTO.fromEntity(community));
        keywordService.delete(CommunityDTO.fromEntity(community));
        imageService.delete(CommunityDTO.fromEntity(community));
    ...
    private Community getCommunityByIdOrException(Long communityId) {
        return communityRepository.findById(communityId).orElseThrow(
                () -> new OllehException(ErrorCode.COMMUNITY_DOES_NOT_EXIST));
    }
